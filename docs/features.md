...
code-kanban gives the **conductor** a persistent, file-backed **private kanban board**,
...
- **Cards:** one markdown file per card, with a Goal, Acceptance checklist, and an append-only
  Logbook. IDs are server-assigned, per-project, sortable (`2026-0042`).
- **Plans:** a card may carry an optional **link to a plan file** (never the plan text), so work
  planned days earlier outlives the worker that planned it — a parked-with-plan card is just `todo`
  plus that link. Set it at filing time (`file_card`'s `plan`) or later with `update_card`. Two
  ...
  live pointer. Input forms, the stored form and refusals: `docs/protocol.md` (`update_card`).
- **Priority:** a card carries one of `CRITICAL`, `HIGH`, `MEDIUM`, `LOW`, or is **unset**. Unset is
  ...
  worker-filed card until someone reviews it. Omitting `priority` on `file_card` leaves the card
...
- **Workers are pure emitters** — only `file_card` and `log_card`, no reads. A worker never
