...
| `GET /api/board/:project/epics` | `board.listEpics` | — | `{ok, epics:[{slug, title, rollup, projects, completed}]}` (incl. cross-project epics spanning the project), ordered active before completed, then last activity descending, then slug — same rule as `list_epics` above |
