...
`update_card` splits `fields` in two. `UPDATABLE` (`src/board.js:731`) lists every accepted key;
`PRE_RESOLVED` (`src/board.js:735`) lists the ones with their own resolver above the loop. Everything
in the difference is assigned **verbatim** by the generic loop at `src/board.js:805` — that verbatim
...
`resolvePlanForSet` (`src/board.js:774`) is the **only** prologue step with a side effect: an
...
One known gap in this same prologue is tracked as board card `2026-0032`. `2026-0033` closed the
other: `checkTitle` now also refuses a newline, because a `title` newline does not truncate but
