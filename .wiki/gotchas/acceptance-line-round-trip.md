...
(`taskfile.serializeBody`, `src/taskfile.js:62-63`), and `taskfile.parse` **trims each line**
before matching it against the checkbox regex (`src/taskfile.js:115-118`). Two consequences that
...
## Why `update_task`'s acceptance validator refuses both

`update_task`'s `fields.acceptance` validator (`cleanAcceptanceText` in `src/board.js`, feeding
...
## `file_task` is knowingly asymmetric

`file_task`'s `acceptance: string[]` filing-time form (`src/board.js:286`) does **not** run through
this validator — it accepts any string, untrimmed, newline included. So `update_task` can refuse a
criterion that `file_task` happily accepted at filing time. This is intentional for 2026-0020 (the
owner is tracking the asymmetry separately, not fixing it here): the two input forms are deliberately
not unified — see `docs/protocol.md`'s `update_task` acceptance section for the full contract, and
