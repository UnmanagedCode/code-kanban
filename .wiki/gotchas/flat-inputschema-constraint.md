...
- `update_card.fields` is an object with inner keys — **not expressible**. It is advertised as an
  opaque `{type:"object"}` and its keys validated at runtime in `board.updateCard`. (Same trick
