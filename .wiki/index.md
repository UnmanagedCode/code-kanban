...
- [architecture/cross-instance-sync.md](architecture/cross-instance-sync.md) — two-click one-way pull; hidden `uid`/`updated`/`node` stamp; deterministic legacy-uid backfill (why shared-lineage matches); LWW + display-id reassignment; depends_on translation; no-auth trust model.
