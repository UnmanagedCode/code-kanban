...
- [gotchas/update-card-verbatim-fields.md](gotchas/update-card-verbatim-fields.md) — the `UPDATABLE`/`PRE_RESOLVED` split and which `update_card` fields land verbatim; what a wrong-typed `goal` (throws out of the lock) vs `title` (silently stringified, or titleless) does on disk; why a new field validator belongs above `resolvePlanForSet`, the only side-effecting prologue step.
