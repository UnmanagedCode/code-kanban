...
- [gotchas/acceptance-line-round-trip.md](gotchas/acceptance-line-round-trip.md) — a criterion is one `- [ ] <text>` line; a newline silently truncates it and empty-after-trim text vanishes, why `update_card`'s validator refuses both and stores trimmed text, and why `file_card` is knowingly asymmetric.
