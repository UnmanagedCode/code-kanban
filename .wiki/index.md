...
- [architecture/card-epic-tool-split.md](architecture/card-epic-tool-split.md) — why card|epic logging is two tools (`log_card`/`log_epic`) and not a `oneOf` union; why `read_progress`' epic arm was deleted rather than kept; the accepted most-recent-first vs chronological ordering gap and `logbook_total` as its compensation; the deferred generic object-ref grammar and its revisit trigger.
