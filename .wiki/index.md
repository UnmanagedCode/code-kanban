...
- [gotchas/result-envelope-vs-ok-shape.md](gotchas/result-envelope-vs-ok-shape.md) — `{result}` outer envelope vs `{ok}` domain payload; refusals are never thrown; the host's `{meta,text}` raw-text channel (every read) and what it costs `body.result` readers; `list_tasks`' MCP-vs-HTTP default disagreement.
...
- [gotchas/acceptance-line-round-trip.md](gotchas/acceptance-line-round-trip.md) — a criterion is one `- [ ] <text>` line; a newline silently truncates it and empty-after-trim text vanishes, why `update_task`'s validator refuses both and stores trimmed text, and why `file_task` is knowingly asymmetric.
...
- [architecture/cross-instance-sync.md](architecture/cross-instance-sync.md) — two-click one-way pull; hidden `uid`/`updated`/`node` stamp; deterministic legacy-uid backfill (why shared-lineage matches); LWW + display-id reassignment; depends_on translation; epic sync (slug-as-identity, NOT uid; kind-conflict skip+log; epics-before-cards); no-auth trust model.
